...
(async () => {
  ...
  const isVolumeByVolumeMode =
    Array.isArray(selectionResult) && selectionResult[0] === 'VOLUME_BY_VOLUME';
  const selectedVolumes = isVolumeByVolumeMode
    ? selectionResult.slice(1)
    : selectionResult;
  ...
  if (isVolumeByVolumeMode) {
    ...
    const chaptersToDownload = $bookService.filterChaptersByVolumes(
      allChapters,
      selectedVolumes
    );
    ...
    const createdFiles = await $bookService.processVolumesByOne(
      chaptersToDownload,
      BOOK_ID,
      bookInfo,
      booksDir
    ...
    if (chaptersToDownload.length > 0) {
      ...
      if (chaptersToDownload.length > 1) {
        console.log(
          `   Последняя глава: "${
            chaptersToDownload[chaptersToDownload.length - 1].title
          }"`
        );
      }
    }
    ...
    await $commonService.delay(1000);
    const bookContent = await $bookService.getAllBookContent(
      chaptersToDownload,
      BOOK_ID
    );
    ...
    if (selectedVolumes.length >= 1 && selectedVolumes[0] !== -1) {
      ...
      if (selectedVolumes.length === 1) {
        ...
        outputFileName += `_тома_${selectedVolumes[0]}-${
          selectedVolumes[selectedVolumes.length - 1]
        }`;
      }
    ...
    try {
      ...
      if (
        ...
        const fallbackPath = OUTPUT_BOOK_PATH.replace(
          '.epub',
