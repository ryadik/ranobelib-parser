...
export class CommonService implements CommonServiceModel {
  ...
  public userAlert(): void {
    console.log(
      'Здравствуйте, это программа загрузки книг с сайта ranobelib.me'
    );
    ...
    console.log(
      'Во время загрузки, пожалуйста, не трогайте окно браузера, это может привести к сбоям.'
    );
  }
  ...
  public getBookURL(): string {
    ...
    console.log(
      'Пример адреса: https://ranobelib.me/sakurasou-no-pet-na-kanojo-novel'
    );
    console.log(
      'Пожалуйста, указывайте верный адрес, во избежания проблем с работой программы.'
    );
  ...
  public selectVolumesToDownload(chaptersData: any[]): any[] {
    ...
    switch (choice) {
      ...
      case '2':
        const volumesInput = prompt(
          `Введите номера томов через запятую (доступно: ${volumes.join(
            ', '
          )}): `
        );
        if (volumesInput) {
          const inputVolumes = volumesInput
            .split(',')
            .map((v) => parseInt(v.trim()))
            .filter((v) => !isNaN(v) && volumes.includes(v));
        ...
        if (rangeInput && rangeInput.includes('-')) {
          ...
          if (!isNaN(start) && !isNaN(end) && start <= end) {
            ...
            if (rangeVolumes.length > 0) {
              console.log(
                `✅ Выбран диапазон: тома ${rangeVolumes.join(', ')}`
              );
      ...
      case '4':
        const maxChaptersInput = prompt(
          'Введите количество первых глав для загрузки: '
        );
    ...
    if (selectedVolumes.length > 1) {
      ...
      console.log(
        '1. 📚 Объединить все в один EPUB (быстрее, но риск потерять всё при ошибке)'
      );
      console.log(
        '2. 🔥 Обрабатывать тома по отдельности (БЕЗОПАСНО! Рекомендуется)'
      );
      ...
      if (modeChoice === '2') {
        ...
        return ['VOLUME_BY_VOLUME', ...selectedVolumes]; // Специальный маркер
      } else {
